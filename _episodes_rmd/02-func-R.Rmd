...
```{r}
center <- function(data, around) {
  new_data <- (data - mean(data)) + around
  return(new_data)
}
```
...
```{r}
center <- function(data, around = 0) {
  # return a new vector containing the original data centered around the
  # given value (0 by default).
  # Example: center(c(1, 2, 3), 0) => c(-1, 0, 1)
  new_data <- (data - mean(data)) + around
  return(new_data)
}
...
The key change is that the second argument is now written `around = 0` instead of just `around`.
...
But we can also now call `center()` with just one argument, in which case `around` is automatically assigned the default value of `0`:
